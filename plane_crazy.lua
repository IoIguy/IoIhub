...
Tab1:Button("Paint aircraft random color", "Paints your vessel random colors every 0.5 seconds.", function()
...
Tab1:Button("Gravity gun", "Gives you a gravity gun that can move only your creation.", function()
...
Tab1:Button("Building grid", "Makes a platform so you dont have to build scaffoldings to place blocks.", function()
...
Tab2:Button("Kill rocketeers", "Kill players using rockets.", function()
	pcall(function()
		while wait() do
			local guns = {}
			for i,v in pairs(game:GetService("Workspace")[game.Players.LocalPlayer.Name .. "Aircraft"]:GetChildren()) do
...
Tab2:Button("Kill bombers", "Kill players using TNT.", function()
	local bomb = {}
...
Tab3:Button("Blackhole", "Make a black hole at your position out of your creation's blocks. Better effect with 1x1 blocks.", function()
...
Tab3:Button("Lag server", "Lag the server you are in to the point where it crashes.", function()
	for i=1, 20 do
		pcall(function()
			parachute()
		end) 
...
Tab3:Button("Disable anti copy blacklist", "Disable the blacklist and make anti copy blocks visible again.", function()
...
Tab3:Button("Infinite terrain generation", "Infinite terrain generation (client sided). Does not work well on low-end PCs.", function()
...
Tab3:Textbox("Change thruster speed", "Change all movement speed by X%. Default - 100%.", true, function(text)
...
Tab3:Button("Ocean", "Turn the map into ocean and islands (client sided).", function()
...
Tab3:Button("Kill bombers", "Kill players using TNT in normal servers.", function()
	local bomb = {}
	local Players = game:GetService("Players")
	local remotes = game:GetService("ReplicatedStorage"):WaitForChild("Remotes")
	local explode = remotes:WaitForChild("Explode")
...
Tab3:Button("Explode TNT", "Explode TNT while in non-PVP servers.", function()
	local RunService = game:GetService("RunService")
	local Players = game:GetService("Players")
...
Tab4:Button("Build canvas", "Build a full size canvas out of 1x1 blocks.", function()
	for x=1, 51, 1 do
		for y=1, 39, 1 do
			spawn(function()
...
Tab4:Button("Build canvas (hover engines)", "Build a full size canvas out of hover engines.", function()
	for x=1, 51, 1 do
		for y=1, 39, 1 do
			spawn(function()
				local A_1 = Vector3.new(x-26, y, 25)
...
Tab4:Button("Build canvas (snow blasters)", "Build a full size canvas out of snow blasters.", function()
	for x=1, 51, 1 do
		for y=1, 39, 1 do
			spawn(function()
				local A_1 = Vector3.new(x-26, y, 25)
...
Tab4:Button("Rainbow vessel", "Paint your vessel into random colors.", function()
...
Tab5:Textbox("Video file", "Play a video in your executor's workspace folder.", true, function(text)
	print(tostring(text))
	local checkloop = 0
	local waitloop = 0
	for assad2,playerzone in pairs(game.Workspace.BuildingZones:GetChildren()) do
...
Tab5:Textbox("Video inputstream", "Currently not for use.", true, function(text)
	local checkloop = 0
	local waitloop = 0
	for assad2,playerzone in pairs(game.Workspace.BuildingZones:GetChildren()) do
		pcall(function()
